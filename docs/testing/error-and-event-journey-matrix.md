...
| `ORDER_NOT_FOUND` | Hidden/missing order or unknown delivery-provider order ID | Covered in get-order, status, and webhook tests | None | Reproduce on API and webhook routes |
...
| `MERCHANT_ORDER_ID_CONFLICT` | Same merchant order ID with a different idempotency key | Covered at HTTP and DynamoDB integration boundaries | None | Reproduce through `POST /orders` |
